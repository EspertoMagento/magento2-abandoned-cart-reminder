...
namespace EspertoMagento\AbandonedCartReminder\Model;

use EspertoMagento\AbandonedCartReminder\Helper\AbandonedHelper;
...
class CronProcessor
{
    ...
    public function runCronjobTask(): void
    {
        try {
            foreach ($this->storeManager->getStores() as $store) {
                $cartAbandonmentPeriod = $store->getConfig('em_abandoned_configuration/abandoned_cart/duration');
    ...
    private function getActiveCartsForStore(
        ...
        try {
            ...
            $consentedConfig = (bool) $store->getConfig('em_abandoned_configuration/abandoned_cart/consented');
            if ($consentedConfig) {
